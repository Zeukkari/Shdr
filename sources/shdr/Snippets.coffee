Snippets =
  ...
  'TestailuVertex': [
    ...
    '  vec4 pos = modelViewMatrix * vec4(position, 2.0);'
    '  fPosition = pos.xyz;'
    ''
    ''
    '  float animationframes = sin(time*20.0)+1.0;'
    '  float offsetter2 = sin(fNormal.y*sin(time))+sin(fNormal.x*sin(time*time)*0.01)+1.0;'
    '  float offsetter3 = sin(fNormal.y*sin((time*time)/5.0)+1.0)*cos(fNormal.x*sin((time*time)/5.0)+1.0);'
    '  '
    '  '
    '  vec4 samplePos1 = projectionMatrix * vec4(pos.x*fNormal.y, pos.y*(fNormal.z+fNormal.x), pos.z*(fNormal.y), pos.w);'
    '  vec4 samplePos2 = projectionMatrix * pos;'
    ''
    '  vec4 samplePos3 = samplePos2+(animationframes+0.5)*vec4(fNormal.y*fPosition.y, fNormal.x*fPosition.x, fNormal.z*fPosition.z, 1.0);'
    '  vec4 samplePos4 = samplePos2 + (animationframes-1.0)/10.0*vec4(samplePos2.x, -samplePos2.y, samplePos2.z*-1.0, -1.0*samplePos2.w*sin(animationframes))*offsetter2/(fNormal.y/fNormal.z);'
    ''
    '  float bias1 = abs(animationframes-1.5)*0.1;'
    '  float bias2 = abs(animationframes-1.0)*(sin(time)*-cos(time));'
    ''
    '  if(animationframes<1.5) gl_Position = samplePos2 + 0.5*(samplePos3*bias1 + samplePos4*bias2);'
    '  if(animationframes<1.0) gl_Position = samplePos2 + 0.1*(samplePos4*bias1 + samplePos3*bias2);'
    '  if(animationframes>1.5) gl_Position = samplePos2 + bias2*(samplePos3);'
    '}'
  ].join('\n')

  'TestailuFragment': [
    'precision highp float; uniform float time; uniform vec2 resolution;'
    'uniform vec4 iMouse;'
    ...
    '}'
    ''
    'void juliaSet( out vec4 fragColor, in vec2 fragCoord )'
    '{'
    '    '
    '    // compute coordinate '
    ...
    '	}'
    '	'
    '    if (count == i_max-1) {fragColor = vec4(1.0, 0.0,0.0,1.0);}'
    '    else  fragColor = vec4(1.0- float( count ) * scale );'
    '}'
    ''
    ...
    '   return sin( p*6.2831853 + timeOffset )*(sin(timeOffset)+1.0)*.5 + .5;'
    '}'
    ''
    ''
    'vec2 cellPt(vec2 p) {'
    ...
    '}'
    ''
    'vec2 circumCircle(vec2 h1, vec2 h2, vec2 h3)'
    '{'
    '   vec2 u = h2 - h1;'
    '   vec2 v = h3 - h1;'
    '   float d = u.x*v.y - u.y*v.x;'
    '   vec2 c = vec2(dot(u,u), dot(v,v)) * (.5 / d);'
    '   return vec2(c.x*v.y - u.y*c.y, u.x*c.y - c.x*v.x);'
    '}'
    ''
    ...
    ''
    '   vec2 c = circumCircle(h0, h1, h3);'
    '   vec2 dh = h2 - h0 - c;'
    '   float dc = dot(dh, dh) - dot(c,c);'
    ''
    '   #if FLIP_ANIMATION'
    '     float f = clamp(dc*4. + .5, 0., 1.);'
    '   #else'
    ...
    '  float dist = tri; // ev.y; //(iMouse.z > 0.) ? ev.y : tri;'
    '  vec3 col = shade(tri, tri, ev, ss);'
    '  vec3 col2 = shade(tri, ev.y, ev, ss);'
    '  fragColor = vec4(gamma(col), 1);'
    '  fragColor2 = vec4(gamma(col2), 1.0);'
    '  gl_FragColor = fragColor * (sin(time)+1.0) + fragColor2 * (cos(time*5.0)+1.0);'
    '}'
  ].join('\n')
